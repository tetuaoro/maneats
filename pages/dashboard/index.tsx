...
const Page: NextPage<PageProps> = (props) => {
  const account: AccountData | null = props.account ? JSON.parse(props.account) : null
          ...
          <Col>
            <Card className="shadow">
              <Card.Body>
                ...
                <p>{new Date(account && account.creationdate ? account.creationdate * 1000 : "").toLocaleString()}</p>
              </Card.Body>
            </Card>
          </Col>
          <Col>
            <Card className="shadow">
              <Card.Body>
                ...
                <p>{account && account.enddate ? new Date(account.enddate * 1000).toLocaleString() : "indéterminé"}</p>
              </Card.Body>
            </Card>
          </Col>
        </Row>
...
export const getServerSideProps = async () => {
  try {
    ...
    return { props: { account: JSON.stringify(data) } }
  } catch (error) {
    logger("err", error)
